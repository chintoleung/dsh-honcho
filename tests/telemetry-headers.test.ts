/**
 * Wire test: drive the plugin's real seams and read the headers off the requests
 * the SDK actually sent. Nothing asserts on a formatting helper — a header built
 * correctly and never reaching a request is the failure this is for.
 */
...
function stubContext() {
  ...
  const ctx = {
    ...
    // Capture is wired behind `ctx.inject(["sessionQuery"])`, left unmounted here.
    inject() {},
    effect() {},
    get: () => undefined,
  ...
  /** A model-facing tool call: the awaited path to a request. */
  const search = () =>
...
const sessionEvent = (type: string, data: unknown, id = "session-1") =>
  ["session/event", { id }, { type, data }] as const;

const preStep = (fire: ReturnType<typeof stubContext>["fire"], id = "session-1") =>
  fire(
    "agent/pre-step",
    { agent: { session: { id, header: { cwd: "/tmp/project" } } }, messages: [], step: 1 },
...
test("every request carries host, plugin, and the model that is answering", async () => {
  ...
  globalThis.fetch = (async (_url: string, init: RequestInit = {}) => {
    ...
    // Enough body for the SDK to build a peer, session and context from.
  ...
  try {
    ...
    await search();
    const afterAssistant = sent.length;

    // A turn for a session that has not answered yet: the header must come off,
    // not keep the previous session's model.
    await preStep(fire, "session-2");
    await search();
    ...
    expect(sent.length).toBeGreaterThan(afterAssistant);
    // Removing the model must not disturb the two headers that never change.
    ...
    for (const headers of sent.slice(afterHeader, afterAssistant)) {
      expect(headers.get("X-Honcho-Agent-Model")).toBe("openrouter/kimi-k2");
    }
    for (const headers of sent.slice(afterAssistant)) expect(headers.get("X-Honcho-Agent-Model")).toBeNull();
  } finally {
