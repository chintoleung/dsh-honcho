...
| `X-Honcho-Agent-Model` | `deepseek/deepseek-chat` | Durable session events: `assistant/message` carries the provenance of what the model actually produced, `request/header` names the configured model one request earlier and on every mid-session switch. Omitted until one of them has been seen. |

The client is held across turns, so the model cannot be baked in at construction. `createGateway` takes an
accessor and merges the current identity into `honcho.http.defaultHeaders` on every client access; the SDK
reads that map per request. Model is tracked **per dsh session** because global `session/event` listeners see
subagent sessions too — the reported model is the one for the session whose turn is being served.
